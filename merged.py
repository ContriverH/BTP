...
def trafficCounter(cordinates):
    ...
    image2 = cv2.resize(frame, (0, 0), None, ratio, ratio)  # resize image 
    ...
    while True:
        ...
        if ret:  # if there is a frame continue with code
            ...
            cv2.rectangle(image2, (0, 0), (250, 100), (255, 0, 0), -1)  # background rectangle for on-screen text

            cv2.putText(image2, "Cars in Area: " + str(currentcars), (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)

            cv2.putText(image2, "Cars Crossed Up: " + str(carscrossedup), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0),
                        1)

            cv2.putText(image2, "Cars Crossed Down: " + str(carscrosseddown), (0, 45), cv2.FONT_HERSHEY_SIMPLEX, .5,
                        (0, 170, 0), 1)

            cv2.putText(image2, "Frame: " + str(framenumber) + ' of ' + str(frames_count), (0, 75), cv2.FONT_HERSHEY_SIMPLEX,
                        .5, (0, 170, 0), 1)

            cv2.putText(image2, 'Time: ' + str(round(framenumber / fps, 2)) + ' sec of ' + str(round(frames_count / fps, 2))
